// test/checkbox-logs-final-test.js
// Capture browser logs to see connectedCallback calls

const { chromium } = require('playwright');

async function testLogsFinal() {
  const browser = await chromium.launch({ headless: false });
  const page = await browser.newPage();
  
  try {
    console.log('🔍 Capturing browser logs for connectedCallback...');
    
    // Capture console logs
    const logs = [];
    page.on('console', msg => {
      logs.push(msg.text());
      if (msg.text().includes('CONNECTED_CALLBACK') || msg.text().includes('CLICK_EVENT')) {
    ...
    if (await indeterminateCheckbox.count() > 0) {
      ...
      console.log('Logs after click:', logs.filter(log => 
        log.includes('CONNECTED_CALLBACK') || 
        log.includes('CLICK_EVENT')
      ));
